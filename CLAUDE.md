...
- Tests: `vitest`. Lint: `eslint` 9. Version `0.3.0` (`package.json`).
...
Do all CLI work here. In-flight: `fix/apps-permissions-*` branches migrate apps `role` →
granular `permissions[]` to match the API contract (not yet merged to `main`).
